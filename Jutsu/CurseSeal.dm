...
obj/CurseSeal
	verb
		Unlock_Power()
			set category="Jutsus"
			if(!usr.CurseSeal&&usr.HasCurseSeal)
			...
			if(!usr.Earth_CurseSeal&&usr.Earth_HasCurseSeal)
				usr.overlays += 'CurseSealAura.dmi'
				sleep(10)
			...
			if(!usr.Stone_CurseSeal&&usr.Stone_HasCurseSeal)
				usr.overlays += 'CurseSealAura.dmi'
				sleep(10)
			...
			if(!usr.Stone_CurseSeal2&&usr.Stone_HasCurseSeal2)
				...
				sleep(8)
				usr.icon = 'Stone2.dmi'
				usr.hair = rgb(204,105,0)
				usr.Poison2()
				return


			if(!usr.Heaven_CurseSeal&&usr.Heaven_HasCurseSeal)
				usr.overlays += 'CurseSealAura.dmi'
				sleep(10)
...
mob/proc
	Curse()
		if(src.HasCurseSeal==0)
			viewers(src) << "<font color=silver><b>[src] has been bitten"
			src.Kyuubi2_Revert()
			src.Kyuubi1_Revert()
			src.Poison=1
			spawn(30)
		...
		if(random==2)
			...
			sleep(300)
			viewers(src) << "<font color=silver><b>[src]'s body Couldn't handle the Curse Seal"
			sleep(10)
			src.Death()
			src.Poison=0
...
mob/proc
	Poison()
		if(src.Poison)
			src.JConst -= rand(10,15)
			if((src.JConst/src.maxJConst) <= 50)
				src.Health = 0
				src.Ko()
			spawn(rand(30,60))
			src.Poison()
	Poison2()
		if(src.Poison)
			src.JConst -= rand(5,10)
			if((usr.JConst/usr.maxJConst) <= 50)
				usr.Health = 0
				usr.Ko()
			spawn(rand(50,100))
			src.Poison2()
