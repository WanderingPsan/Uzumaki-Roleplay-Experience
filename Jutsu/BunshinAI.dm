	...
	BunshinAttack()
		...
		for(var/mob/M in get_step(src,dir))
			if(!src.KO&&!M.KO&&M.attackable&&!src.attacking&&src.Stamina >= (src.MaxStamina/50))
				...
				var/damage = rand((src.Strength-M.Defense)-src.Taijutsu,(src.Strength-M.Defense)+src.Taijutsu)
				if(damage<0)
					damage=0
				if(damage*1.5>=M.MaxHealth)
					damage=(usr.Strength/M.Defense+M.Taijutsu)/1.5
				if(damage*1.5>=500)
					damage=(usr.Strength/M.Defense+M.Taijutsu)*rand(1.5,2.5)
				src.attacking=1
				if((src.Agility*rand(1,2)) >= M.Agility)
					random = rand(1,10)
					if(random<=9)
						if((src.Strength*rand(1,2)) >= M.Defense)
							random = rand(1,10)
							if(random<=9)
								viewers() << "[src] manages to hit [M]!"
								flick("Attack",src)
								M.Health -= damage

							else

								viewers() << "[M] manages to block [src]!"
								flick("Attack",src)
						...
						if(random<=8)
							...
							viewers() << "[M] manages to dodge [src]!"
							flick("Dodge",M)
							flick("Attack",src)
				else
					viewers() << "[M] manages to dodge [src]!"
