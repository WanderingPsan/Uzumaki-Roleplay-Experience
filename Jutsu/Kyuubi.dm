		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(prob(10/skill/skill/skill))
					usr<<"Your understanding of Kyuubi's Chakra has increased."
					skill+=1
				usr.firing=1
				src.Active=1
				...
				viewers(16) << "<font color=[usr.SayFont]>[usr] yells<font color = white>: Rawr!"
		...
		if(src in usr.contents)
			if(usr.move)
				...
				if(usr.Chakra<usr.MaxChakra*(0.30/skill))
					return
				if(prob(10/skill/skill/skill))
					usr<<"Your understanding of Kyuubi's Roar has increased."
					skill+=1
				usr.firing=1
				src.Active=1
				usr.MaxNinjutsu += (1 *0.001)
