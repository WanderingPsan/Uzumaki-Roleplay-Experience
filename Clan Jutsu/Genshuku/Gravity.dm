...
obj/Doujutsu/Gravity_Eyes
	...
	Click()
		if(src in usr.contents)
			if(!usr.Gravityeye)
				usr.Gravityeye=1
				viewers() << "<font color = white>(*<font color=[usr.SayFont]>[usr]'s eyes turn purple as the gravity around them intensifys<font color = white>*)"
				usr.overlays += 'Gravity Eyes.dmi'
			else
				usr.Gravityeye=0
				viewers() << "<font color = white>(*<font color=[usr.SayFont]>[usr]'s eyes return to normal as the purple fades<font color = white>*)"
				usr.overlays -= 'Gravity Eyes.dmi'
			...
			if(src in usr.contents)
				if(usr.move)
					...
					if(usr.Ninjutsu >= A.Ninjutsu*1.3)
						...
						viewers(16) << "<font color=[usr.SayFont]>[usr] yells<font color = white>: Genshuku Distortion no Jutsu!"
